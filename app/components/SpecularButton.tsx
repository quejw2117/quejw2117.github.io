...
export default function SpecularButton({
  ...
  useEffect(() => {
    ...
    const onPointerMove = (event: PointerEvent) => {
      ...
      const proximityValue = Math.max(
        0,
        1 - distance / Math.max(propsRef.current.proximity, 1),
      );
      proximityAmount =
        proximityValue * proximityValue * (3 - 2 * proximityValue);
    };
    ...
    const update = (now: number) => {
      ...
      idleAngle += current.speed * deltaTime;
      const shouldSteer =
        current.followMouse &&
        pointerAngle !== null &&
        (!current.autoAnimate || proximityAmount > 0);
      const targetAngle = shouldSteer ? pointerAngle! : idleAngle;
      const difference =
      ...
      brightness +=
        (targetBrightness - brightness) * (1 - Math.exp(-deltaTime * 8));
      ...
      program.uniforms.uRadius.value =
        Math.min(
          current.radius,
          Math.min(sizeRef.width, sizeRef.height) / 2,
        ) * dpr;
